-- Anthropic OAuth adapter for CodeCompanion
-- Provides OAuth 2.0 + PKCE authentication for Anthropic Claude API
...
local OAUTH_CONFIG = {
  ...
  TOKEN_FILE = "anthropic_oauth.json",
}
...
---Ensure beta flag is present in headers
---@param headers table
---@param flag string
...
local function get_token_file_path()
  return oauth_utils.get_token_file_path(OAUTH_CONFIG.TOKEN_FILE, "CODECOMPANION_ANTHROPIC_TOKEN_PATH")
end
...
local function save_api_key(api_key)
  ...
  log:error("Anthropic OAuth: Failed to save API key")
  return false
end
...
local function create_api_key(access_token)
  ...
  if response.status >= 400 then
    log:error("Anthropic OAuth: Failed to create API key, status code %d: %s", response.status, response.body or "no body")
    return nil
  end
...
local function generate_auth_url()
  local pkce = oauth_utils.generate_pkce(128)
  if not pkce then
    return nil
  end
  ...
  local query_params = {
    ...
    "state=" .. oauth_utils.url_encode(pkce.verifier),
  }
...
function M.show_status()
  local api_key = load_api_key()
  if not api_key then
    vim.notify("Anthropic API key not found. Run :CCTools adapter anthropic auth", vim.log.levels.WARN)
    return
  end
...
function M.create_adapter()
  ...
  -- Override existing default models with latest models
    ...
    form_parameters = function(self, params, messages)
      ...
      if model_opts and model_opts.opts and model_opts.opts.max_output then
        if params.max_tokens and params.max_tokens > model_opts.opts.max_output then
          log:debug("Model %s max_tokens %d exceeds maximum %d, adjusting to maximum", model, params.max_tokens, model_opts.opts.max_output)
      ...
      if model_opts and model_opts.opts then
        ...
        if model_opts.opts.can_reason then
          if not string.find(self.headers["anthropic-beta"], "interleaved%-thinking") then
            self.headers["anthropic-beta"] = self.headers["anthropic-beta"] .. ",interleaved-thinking-2025-05-14"
            log:debug("Enabled thinking support for model %s", model)
          end
        else
          if string.find(self.headers["anthropic-beta"], "interleaved%-thinking") then
            self.headers["anthropic-beta"] = self.headers["anthropic-beta"]:gsub(",?interleaved%-thinking%-[^,]*", "")
