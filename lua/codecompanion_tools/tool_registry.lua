...
function M.register_tools(tools, debug)
  ...
  local registration_targets = {
    {
      ...
      loader = function() return require("codecompanion.config") end,
      validator = function(target) return target.strategies and target.strategies.chat end
    },
    {
      name = "codecompanion module",
      loader = function() return require("codecompanion") end,
      validator = function(target) return target.config end,
      config_path = function(target) return target.config end
    }
  }
  ...
  for _, target in ipairs(registration_targets) do
    ...
    if ok and target.validator(config_obj) then
      ...
      if not config.strategies.chat.tools then config.strategies.chat.tools = {} end
